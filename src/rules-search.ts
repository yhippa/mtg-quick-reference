...
import type { BM25Index, RuleDocument, RuleHit, RulesHits } from './rules-types.ts';
...
export function tokens(text: string): string[] { return normalize(text).split(' ').filter(t => t && !stop.has(t)); }
export function buildBM25(docs: { title: string; text?: string; aliases?: string[] }[]): BM25Index {
  const postings: Record<string, number[]> = Object.create(null), lengths: number[] = [];
  docs.forEach((d, i) => {
    const title = tokens([d.title, ...(d.aliases ?? [])].join(' ')), body = tokens(d.text ?? '');
    const tf = new Map<string, number>();
    for (const t of title) tf.set(t, (tf.get(t) ?? 0) + 3);
    for (const t of body) tf.set(t, (tf.get(t) ?? 0) + 1);
    lengths.push(title.length * 3 + body.length);
    for (const [t, n] of tf) (postings[t] ??= []).push(i, n);
  });
  return { version: 1, n: docs.length, avgLength: lengths.reduce((a,b) => a+b,0)/docs.length, lengths, postings };
}
export function queryBM25(index: BM25Index, query: string, limit = 10, minDoc = 0) {
  ...
  for (const term of terms) {
    ...
    for (let j=0;j<list.length;j+=2) {
      const id=list[j],tf=list[j+1]; if(id<minDoc) continue;
      const score=idf*(tf*2.2)/(tf+1.2*(.25+.75*index.lengths[id]/index.avgLength));
      scores.set(id,(scores.get(id)??0)+score); coverage.set(id,(coverage.get(id)??0)+1);
    }
  }
  for(const [id,score] of scores) scores.set(id,score*(coverage.get(id)!/terms.length)**2);
  return [...scores].sort((a,b)=>b[1]-a[1]||a[0]-b[0]).slice(0,limit).map(([i,score])=>({i,score}));
}
export const compactRule = ({id,kind,key,title}: RuleDocument): RuleHit => ({id,kind,key,title});
export function createRulesSearch(docs: RuleDocument[], index: BM25Index) {
  const direct = new Map(docs.map(d=>[d.key.toLowerCase(),d]));
  const titles = new Map<string, RuleDocument[]>();
  for (const d of docs) if(d.kind!=='rule') { const title=normalize(d.title); titles.set(title,[...(titles.get(title)??[]),d]); }
  return (q: string): RulesHits => {
