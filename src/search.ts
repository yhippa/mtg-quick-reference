...
export function suggest(index: ReturnType<typeof makeIndex>, query: string, limit = 5): number[] {
  const q = normalize(query);
  if (q.length < 4 || q.length > 100 || search(index, q, 1).length) return [];
  const hits = index.filter(entry => entry.names.some(({ name, words }) =>
    near(q, name) || (!q.includes(' ') && words.some(word => near(q, word)))));
