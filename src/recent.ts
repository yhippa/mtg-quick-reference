...
export function readRecent(storage: RecentStorage): string[] {
  try {
    ...
    return Array.isArray(value) ? [...new Set(value.filter((name): name is string => typeof name === 'string' && name.length > 0 && name.length <= 500))].slice(0, 8) : [];
  } catch { return []; }
}
export function remember(names: string[], name: string): string[] {
  return [name, ...names.filter(n => n !== name)].slice(0, 8);
}
