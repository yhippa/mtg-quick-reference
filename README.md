...
Back navigation preserves the query; card links use hash routes so static hosting needs no URL rewrites. Enter opens the first result, Escape returns from details, and `/` focuses search. Mana costs, Oracle text and rulings share inline visual symbols with accessible labels (including tap/untap, colorless, hybrid and Phyrexian mana). Symbols use the locally bundled [Mana font](https://mana.andrewgioia.com/) by Andrew Gioia, including the card-style tap and colorless glyphs. The font and CSS are included in the offline snapshot; no CDN requests are required. Font (SIL OFL 1.1), stylesheet (MIT), attribution and source revision are in `public/fonts/mana/`; unknown notation stays readable. All source content is HTML-escaped. Autofocus is requested, but mobile browsers may require a tap before showing the keyboard.
